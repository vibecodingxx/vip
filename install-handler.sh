...
if screen -ls | grep -q "\.install_"; then
    for PID in $(pgrep -f "SCREEN.*Install-secure.sh"); do
        if ps -p $PID -o args= | grep -q "/root/Install-secure.sh"; then
            echo "⛔ Install-secure.sh masih berjalan di screen. Tidak boleh menjalankan lebih dari satu instance." | tee -a "$LOGFILE"
            exit 1
        fi
    done
fi
...
if [[ ! -f /root/Install-secure.sh ]]; then
    wget -q https://raw.githubusercontent.com/vibecodingxx/vip/main/Install_secure.sh -O /root/Install-secure.sh
    chmod +x /root/Install-secure.sh
fi

# === Jalankan Install.sh di dalam screen ===
screen -dmS "${SCREEN_NAME}" bash -c "/root/Install-secure.sh ${NAMA} ${DOMAIN} | tee ${LOGFILE}"
